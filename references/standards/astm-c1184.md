---
title: "ASTM C1184 — Structural Silicone Sealants 來源 dossier"
verification_status: "VERIFIED_PRIMARY"
verified_at: "2026-09-02"
document_type: "reference-standard-dossier"
standard_id: "ASTM C1184"
organization: "ASTM International"
current_status: "Active"
edition: "C1184-23"
---

# ASTM C1184 — Structural Silicone Sealants 來源 dossier

- 驗證狀態：`VERIFIED_PRIMARY`
- 標準：ASTM C1184
- 組織：ASTM International
- current edition：C1184-23
- 狀態：Active（2026-09-02 查證）
- 官方來源：https://store.astm.org/c1184-23.html
- 查證日期：2026-09-02
- authority type：industry standard / material specification

## 適用範圍路由（Scope routing）

本標準為 structural silicone sealant 的產品／材料規格之一。它不直接等於特定專案 structural bite、sealant thickness、allowable design stress 或 completed SSG system approval。

## 知識路由（Knowledge routing）

- [`../../knowledge/sealants/structural-silicone-baseline.md`](../../knowledge/sealants/structural-silicone-baseline.md)
- [`../../knowledge/sealants/structural-silicone-bite-routing.md`](../../knowledge/sealants/structural-silicone-bite-routing.md)

## 不可推論事項

- `C1184 compliant` 不代表某個 joint geometry 已設計通過。
- 本 dossier 不重製 ASTM 正式標準全文／表格／試驗程序。

> current edition / status 由本 dossier 維護；knowledge pages 不再各自保存版本快照。